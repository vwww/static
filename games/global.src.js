...
var core = {
	...
	// AJAX POST FUNCTION
	AjaxPost: function (url, param, success_function) {
		var xmlHttp = this.GetXmlHttpObject();
		if (xmlHttp == null) {
			if(!this.ajaxNotified)
				alert("Your browser doesn't support AJAX. You should really upgrade it!");
...
var pub = {
	...
	newCaptcha: function(caller){
		caller.src = caller.src.replace(/&time=$/, '&time=' + new Date().getTime());
	},
	// get comments
